...
> Stale projects generated under earlier names may exist locally (from before the RaiUsage rename). They are also generated and gitignored. Ignore them; `RaiUsage.xcodeproj` is the live one.

Two targets:

| Target | What it is | Sources |
|--------|-----------|---------|
| `RaiUsageApp` | The app host. Non-sandboxed (since v5.0) so it can read the token. `LSUIElement` (no Dock icon). | `RaiUsageApp/` + `Shared/` |
| `RaiUsageTests` | Swift Testing bundle, unsigned. | `RaiUsageTests/` + `Shared/` + the single file `RaiUsageApp/Onboarding/OnboardingViewModel.swift` |
...
- `RaiUsageApp/App/RaiUsageApp.swift` - `@main` (`RaiUsageApp`), the `AppDelegate`, and store wiring.
...
- `RaiUsageApp/App/StatusBarController.swift` - menu bar item and popover hosting.
...
> External contributors: this hardcodes the maintainer's Team ID (`DEVELOPMENT_TEAM=S7B8M9JYF4`), which will not work on your machine. Either replace it with your own (Xcode > Settings > Accounts), or skip signing with `CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO`. Local Release builds are signed ad-hoc / with your Apple Development identity and are not notarized, so Gatekeeper blocks the first launch (right-click > Open, or System Settings > Privacy & Security > Open Anyway). The DMG from [Releases](https://github.com/RoodsBurger/RaiUsage/releases/latest) is also ad-hoc signed; see [`README.md`](README.md) for the Open Anyway flow.

```bash
# Build
xcodegen generate && \
xcodebuild -project RaiUsage.xcodeproj -scheme RaiUsageApp -configuration Release -derivedDataPath build -allowProvisioningUpdates DEVELOPMENT_TEAM=S7B8M9JYF4 build 2>&1 | tail -3 && \
\
